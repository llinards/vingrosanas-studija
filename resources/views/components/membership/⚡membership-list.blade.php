...
new class extends Component {
    ...
    public function memberships(): LengthAwarePaginator
    {
        return Membership::query()
            ...
            ->withCount([
                'bookings' => fn($query) => $query->whereNotIn('payment_status', [PaymentStatus::Refunded, PaymentStatus::Failed]),
            ])
    ...
    public function destroy(Membership $membership): void
    {
        try {
            ...
            Flux::toast(text: __('Abonements veiksmīgi dzēsts!'), variant: 'success');
        } catch (\Exception $e) {
            Log::error($e);

            Flux::toast(text: __('Neizdevās dzēst abonementu. Lūdzu, mēģini vēlreiz.'), heading: __('Kļūda!'), variant: 'danger');
        }
    }
};
?>
    ...
    @else
        <div class="mb-6 flex flex-col gap-4">
            <flux:input prefix-icon="magnifying-glass" type="search" wire:model.live.debounce.300ms="search"
                placeholder="{{ __('Meklēt abonementus') }}" />
            ...
            <div class="flex flex-wrap items-end gap-4">
                ...
                <flux:select wire:model.live="paymentStatus" :label="__('Maksājuma statuss')">
                    ...
                    @foreach (PaymentStatus::cases() as $paymentStatusCase)
                        <flux:select.option :value="$paymentStatusCase->value">{{ $paymentStatusCase->label() }}
                        </flux:select.option>
                    @endforeach
                </flux:select>
        ...
        @else
                ...
                <flux:table.rows>
                    @foreach ($this->memberships as $membership)
                        <flux:table.row wire:key="membership-{{ $membership->id }}">
                            ...
                            <flux:table.cell>
                                <flux:badge size="sm"
                                    :color="match($membership->payment_status->value) {
                                                                        'paid' => 'green',
                                                                        'pending' => 'yellow',
                                                                        'failed' => 'red',
                                                                        'refunded' => 'zinc',
                            ...
                            <flux:table.cell>
                                <div class="flex gap-2">
                                    <flux:button href="{{ route('admin.memberships.edit', $membership) }}"
                                        variant="primary" size="sm" icon="pencil">
                                    </flux:button>
            ...
            <flux:pagination :paginator="$this->memberships" />
