...
<nav>
    ...
    <div x-cloak x-show="open" x-transition:enter="transition-all duration-300 ease-out"
        ...
        <flux:navlist>
            <flux:navlist.item href="#services" @click="open = false">{{ __('Pakalpojumi') }}</flux:navlist.item>
        </flux:navlist>
        <flux:navlist>
            <flux:navlist.item href="#contactForm" @click="open = false">{{ __('Kontakti') }}</flux:navlist.item>
        </flux:navlist>
