    ...
    <flux:modal name="booking-modal" class="w-full max-w-lg" @close="$wire.resetModal()">
        <div class="space-y-6 md:p-6">
            ...
            <div class="flex items-center justify-center gap-2 mt-12 md:mt-0">
                @for($i = 1; $i <= 4; $i++) <div class="flex items-center gap-2">
                    ...
                    @if($i < 4) <div @class([ 'h-px w-8' , 'bg-green-500'=> $step > $i,
                        'bg-zinc-200' => $step <= $i, ])>
            </div>
            @endif
        </div>
...
@if($step === 1)
<div class="space-y-6">
    <flux:heading size="lg">{{ __('Izvēlieties pakalpojumu') }}</flux:heading>
...
<div class="space-y-6">
    ...
    <div class="flex justify-center">
        <flux:calendar wire:model.live="selectedDate" min="today" :max="now()->addWeeks(4)->toDateString()"
            :unavailable="$this->unavailableDates" locale="lv" start-day="1" />
    </div>

    @if($this->selectedDate && count($this->availableTimeSlots) > 0)
    <flux:radio.group wire:model.live="schedule_id" :label="__('Pieejamie laiki')" variant="cards">
        @foreach($this->availableTimeSlots as $slot)
        <flux:radio :value="$slot['schedule_id']" :label="$slot['start_time'] . ' — ' . $slot['coach_name']"
...
<div class="space-y-6">
    ...
    <div class="rounded-full border border-zinc-200 p-4 space-y-2">
        <div class="flex justify-between">
            <flux:text class="font-medium">{{ __('Pakalpojums') }}</flux:text>
    ...
    <div class="rounded-full border border-zinc-200 p-4 space-y-1">
    ...
    <div class="flex justify-between">
        <flux:button wire:click="previousStep" class="button small tertiary">{{ __('Atpakaļ') }}</flux:button>
        <flux:button wire:click="submitBooking" class="button small primary">{{ __('Apmaksāt') }}</flux:button>
    </div>
</div>
@endif
@endif
